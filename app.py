# app.py (Final - Corrected GY Retention dashboard like Summary cards)
...
def parse_exam_status_report(uploaded_file) -> pd.DataFrame:
    ...
    if tidy.empty:
        raise ValueError("Parsed 0 rows. Please verify file matches Exam Status report structure.")
...
# -----------------------------
# ✅ NEW: GY Retention (Corrected) - Drops total row & builds KPI dashboard
# -----------------------------
@st.cache_data(show_spinner=False)
def load_gy_retention(retention_file) -> pd.DataFrame:
    name = retention_file.name.lower()
    if name.endswith(".xlsx") or name.endswith(".xls"):
        df = pd.read_excel(retention_file)   # first sheet by default
    elif name.endswith(".csv"):
        df = pd.read_csv(retention_file)
    else:
        raise ValueError("Unsupported retention file type. Upload .xlsx/.xls/.csv")

    # Standardize expected columns (based on your file)
    expected = ["Batch", "Programme", "Student Name", "Mail Id", "Lead Type", "Admission Stage", "Fees Paid Mode", "fees"]
    missing = [c for c in expected if c not in df.columns]
    if missing:
        # If structure differs later, still show what columns exist
        raise ValueError(f"Retention file is missing columns: {missing}. Found: {list(df.columns)}")

    # ✅ IMPORTANT FIX:
    # Your file has a last "total" row where Student Name is blank and fees = 4816000.
    # We keep only real student rows (Student Name not null/blank).
    df = df.copy()
    df["Student Name"] = df["Student Name"].astype(str).str.strip()
    df = df[df["Student Name"].notna() & (df["Student Name"] != "") & (df["Student Name"].str.lower() != "nan")].copy()

    # Clean fields
    df["Programme"] = df["Programme"].astype(str).str.strip()
    df["Lead Type"] = df["Lead Type"].astype(str).str.strip()
    df["Admission Stage"] = df["Admission Stage"].astype(str).str.strip()
    df["Fees Paid Mode"] = df["Fees Paid Mode"].astype(str).str.strip()

    # Batch can be date-like
    df["Batch"] = df["Batch"]
...
with st.sidebar:
    ...
    retention_file = st.file_uploader(
        "3) Upload GY Retention file (GY_retention data.xlsx)",
        type=["xlsx", "xls", "csv"],
        key="retention_file"
    )
...
tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8 = st.tabs(
    ["Summary", "Program-wise", "Year-wise", "Agency-wise", "Placement", "Detailed", "SOE Fees Write Off", "GY Retention"]
)
...
# -----------------------------
# ✅ Tab 8: GY Retention (Corrected KPI dashboard)
# -----------------------------
with tab8:
    st.subheader("GY Retention Dashboard")

    if not retention_file:
        st.info("Upload the GY Retention file (GY_retention data.xlsx) from the sidebar to view this tab.")
        st.stop()

    try:
        rdf = load_gy_retention(retention_file)
    except Exception as e:
        st.error(f"Failed to read retention file: {e}")
        st.stop()

    # Create stage bucket
    rdf = rdf.copy()
    rdf["Stage_Bucket"] = rdf["Admission Stage"].apply(_stage_bucket)

    # Sidebar filters just for retention
    st.markdown("### Filters")
    f1, f2, f3 = st.columns(3)
    prog_sel = f1.multiselect("Programme", sorted(rdf["Programme"].dropna().unique().tolist()),
                              default=sorted(rdf["Programme"].dropna().unique().tolist()))
    lead_sel = f2.multiselect("Lead Type", sorted(rdf["Lead Type"].dropna().unique().tolist()),
                              default=sorted(rdf["Lead Type"].dropna().unique().tolist()))
    stage_sel = f3.multiselect("Admission Stage Bucket", sorted(rdf["Stage_Bucket"].dropna().unique().tolist()),
                               default=sorted(rdf["Stage_Bucket"].dropna().unique().tolist()))

    rr = rdf[
        rdf["Programme"].isin(prog_sel) &
        rdf["Lead Type"].isin(lead_sel) &
        rdf["Stage_Bucket"].isin(stage_sel)
    ].copy()

    # KPI Cards (like your main summary)
    total_students = len(rr)
    total_fees = rr["fees"].sum()

    confirmed = rr[rr["Stage_Bucket"] == "Admission Confirmed"]
    prov = rr[rr["Stage_Bucket"] == "Provisionally Admitted"]

    confirmed_count = len(confirmed)
    confirmed_fees = confirmed["fees"].sum()

    prov_count = len(prov)
    prov_fees = prov["fees"].sum()

    retention_pct = (confirmed_count / total_students * 100) if total_students else 0.0

    st.markdown("### Summary")
    c1, c2, c3, c4, c5 = st.columns(5)
    c1.metric("Total Students", f"{total_students}")
    c2.metric("Total Fees (₹)", f"{total_fees:,.0f}")
    c3.metric("Admission Confirmed", f"{confirmed_count}", f"{(confirmed_count/total_students*100 if total_students else 0):.2f}%")
    c4.metric("Confirmed Fees (₹)", f"{confirmed_fees:,.0f}")
    c5.metric("Retention % (Confirmed/Total)", f"{retention_pct:.2f}%")

    st.markdown("---")

    # Charts similar style
    st.markdown("### Programme-wise Fees (₹)")
    prog_fee = rr.groupby("Programme", dropna=False)["fees"].sum().reset_index().sort_values("fees", ascending=False)
    st.dataframe(prog_fee, use_container_width=True, height=300)
    st.bar_chart(prog_fee.set_index("Programme")[["fees"]])

    st.markdown("### Lead Type-wise Fees (₹)")
    lead_fee = rr.groupby("Lead Type", dropna=False)["fees"].sum().reset_index().sort_values("fees", ascending=False)
    st.dataframe(lead_fee, use_container_width=True, height=250)
    st.bar_chart(lead_fee.set_index("Lead Type")[["fees"]])

    st.markdown("### Admission Stage Bucket - Count & Fees")
    stage_summary = (
        rr.groupby("Stage_Bucket", dropna=False)
        .agg(Students=("Student Name", "count"), Fees=("fees", "sum"))
        .reset_index()
        .sort_values("Fees", ascending=False)
    )
    st.dataframe(stage_summary, use_container_width=True, height=250)

    st.markdown("### Detailed Records")
    st.dataframe(
        rr.sort_values(["Programme", "Batch", "Student Name"]),
